...
class HighDimVisualizer:
    ...
    def __init__(self, random_state=42, label_names={}):
    ...
    def plot_pca_2d_3d(self, X, y):
        ...
        axes[0].legend(handles, [f'{self.labels_name[int(l[-3])]}' 
    ...
    def plot_tsne_visualization(self, X, y, n_components=50):
        """
        ...
        for i, perplexity in enumerate(perplexities):
            ...
            if i == 0:
                handles, labels = scatter.legend_elements()
                axes[i].legend(handles[:5], [f'{self.labels_name[int(l[-3])]}' 
                                           for l in labels[:5]], title="Классы", fontsize=8)
    ...
    def plot_parallel_coordinates(self, X, y, feature_names, n_features=10):
        ...
        legend_elements = [Line2D([0], [0], color=colors_plt[i], lw=2, 
                                 label=f'{self.labels_name[int(class_val)]}') 
                          for i, class_val in enumerate(unique_classes)]
    ...
    def plot_radar_chart(self, X, y, feature_names, n_features=6):
        ...
        for idx, class_val in enumerate(unique_classes):
            ...
            axes[idx].set_title(f'{self.labels_name[int(class_val)]}', size=14, y=1.1)
            axes[idx].grid(True)
    ...
    def compare_dim_reduction_methods(self, X, y, PCA_n_components=50):
        """
        ...
        for idx, (name, model) in enumerate(methods):
            ...
            try:
                ...
                if name == 't-SNE' and X.shape[0] > 1000:
                    ...
                    pca_pre = PCA(n_components=PCA_n_components, random_state=self.random_state)
    ...
    def run_full_visualization(self, X, y, feature_names, PCA_n_components=50):
        """
    ...
    def set_data(self, X, y=None, feature_names=None):
        X = np.asarray(X)
        if y is not None:
            y = np.asarray(y)
            if y.ndim > 1:
                y = y.flatten()
            if len(y) != X.shape[0]:
                raise ValueError("Длина y не совпадает с X")
        
        if feature_names is None:
            feature_names = [f'Feature_{i+1}' for i in range(X.shape[1])]
        elif len(feature_names) != X.shape[1]:
            raise ValueError("Количество признаков не совпадает с числом признаков в X")
